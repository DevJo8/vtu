      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Complete solutions for laboratory programming exercises</p>
          </div>
            ...
            <Card key={labTitle}>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  {labTitle}
                  <Badge variant="secondary">Semester {labPrograms[0].semester}</Badge>
                </CardTitle>
                <CardDescription>{labPrograms.length} programs available</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    >
                      ...
                      <CollapsibleContent className="px-4 pb-4">
                        <div className="space-y-4 max-h-96 overflow-y-auto">
                          <div>
                            <h4 className="font-medium mb-2 text-black dark:text-white">Code:</h4>
                            <div className="relative">
                              <ScrollArea className="h-64 w-full rounded-lg border border-zinc-200 dark:border-zinc-800">
                                <pre className="bg-zinc-50 dark:bg-zinc-950 p-4 text-sm">
                                  <code className="text-black dark:text-white">{program.code}</code>
                                </pre>
                              </ScrollArea>
                              <Button
                                size="sm"
                          ...
                          <div>
                            <h4 className="font-medium mb-2 text-black dark:text-white">Expected Output:</h4>
                            <ScrollArea className="h-32 w-full rounded-lg border border-green-200 dark:border-green-800">
                              <pre className="bg-green-50 dark:bg-green-950/20 p-4 text-sm">
                                <code className="text-green-800 dark:text-green-200">{program.expected_output}</code>
                              </pre>
                            </ScrollArea>
                          </div>
                        </div>
        ...
        {Object.keys(groupedPrograms).length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground">No lab programs found for the selected semester.</p>
          </div>
        )}
      </div>
